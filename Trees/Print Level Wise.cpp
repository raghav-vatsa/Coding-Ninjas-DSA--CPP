...
void printLevelWise(TreeNode<int>* root) 
{
    ...
    {
        TreeNode<int>* Current_root = pendingNodes.front();
        pendingNodes.pop();
        cout<<Current_root->data<<":";
        for(int i = 0; i<Current_root->children.size();i++)
        {
            if(i < (Current_root->children.size()-1) )
            {
                cout<<Current_root->children[i]->data<<","; 
            	pendingNodes.push(Current_root->children[i]);
            }
            else
            {cout<<Current_root->children[i]->data;
            	pendingNodes.push(Current_root->children[i]);
